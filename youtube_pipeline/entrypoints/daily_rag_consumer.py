...
LEGACY_OUTPUT_DIR = f"{LEGACY_SIMULATION_DIR}/daily_rag_consumer"
_LEGACY_IDENTITY_PLACEHOLDER = "legacy_daily_rag_consumer"
...
def _read_json_object(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise TypeError("Daily RAG consumer config must be a JSON object.")
    return payload
...
def _component_overrides(
    ...
    result: dict[str, Any] = {"rag": {"daily_consumer": component}}
    if run_id is not None:
        result["rag"]["daily_consumer"]["run_id"] = run_id
    return result


def load_legacy_daily_rag_consumer_config(
    ...
    *,
    ...
    component_payload = payload.get("daily_rag_consumer", payload)
    ...
    if run.rag is None or run.rag.daily_consumer is None:
        raise ValueError("Legacy config did not resolve the daily RAG consumer.")
    return replace(
        run.rag.daily_consumer,
        run_id=_legacy_run_id(effective),
    )


def resolve_daily_rag_consumer_config(
    *,
    config_file: str | Path | None,
    overrides: Mapping[str, Any] | None = None,
    base_dir: str | Path,
    ...
    if resolved.rag is None or resolved.rag.daily_consumer is None:
        raise ValueError("RunConfig must include rag.daily_consumer for this entrypoint.")
    component = resolved.rag.daily_consumer
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        config = resolve_daily_rag_consumer_config(
